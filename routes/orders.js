...
const {
    ...
    checkRequest,
    setOrder
} = require('../logic/OrderCreation');
const {
    getLocations
} = require('../logic/ResponderService');
const {
    calculateDurations
} = require("../logic/MappingService");
const {
    getSiteNumber,
    getTypeNumber
...
// Request resources to be dispatched
router.post("/request-resources", async(req, res) => {
    try {
        ...
        if (!disaster) {
            ...
            res.status(404).json({
                message: `Disaster with ID ${disasterId} not found`
            });
        } else {
